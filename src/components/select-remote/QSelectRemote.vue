<template>
  ...
  >
    ...
    >
      ...
      <q-infinite-scroll ref="infiniteScroll" :handler="__infiniteLoad" inline
...
export default {
  ...
  methods: {
    ...
    __infiniteLoad (index, cb) {
      const done = () => {
        this.loading = false
        cb && cb()
      }

      let freshLoad = this.freshLoad
      this.freshLoad = false
      let offset = freshLoad ? 0 : this.options.length

      if (this.loading) return

      this.loading = true

      Promise.resolve(this.loadMore({term: this.terms, offset})).then(done, done)
    }
  }
}
</script>
